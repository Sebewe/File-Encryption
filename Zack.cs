...
namespace File_Encryption
{
    public class Zack : Crypter
    {
        ...
        {
            var binary = CreateBinary(_message);
                    ...
                    using (var bmp2 = new Bitmap(bmp.Width, bmp.Width))
                    {
                        ...
                        for (var column = 0; column < bmp.Height; column++)
                        {
                            for (var row = 0; row < bmp.Width; row++)
                            {
                                ...
                                {
                                    var tmp = (int)char.GetNumericValue(binary[0]); // gets the first of the string which is the message in binary
                                    binary = binary.Substring(1);  // removes the first value so the loop can iterate through the binary
                                    var newColor = cp.R; // used to read pixel value
                                    Console.WriteLine("s :" + newColor); //debug
                                    switch (tmp)
                                    { // embeds the binary string into the image
                                        case 0:
                                            {
                                                if (cp.R % 2 != 0)
                                                {
                                    ...
                                    Console.WriteLine(binary);  // debug to test deleting values
                                }
                                else
                                {
        ...
        private static string CreateBinary(string message)
        {
            var result = "";
            foreach (var ch in message)
            {
                ...
                result += test;  // causing spaces not to work earlier because it was in the while loop
                                 // it only affected spaces because they needed 2 extra 0 not 1 like most alphabetical chars
            }
            Console.WriteLine(result); //  debug testing if spaces are implemented correctly in encode  (remove in final)
            return result;
        ...
        public sealed override string decryptFile(string bs)
        {
            ...
            {  // if there is still another byte/char and you haven't reached the end sign
            ...
            decryption = decryption.Substring(0, decryption.Length - _end.Length);
